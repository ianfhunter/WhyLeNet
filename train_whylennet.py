...
def get_unit(unit_scalar: Tensor, position_weight: float, digits: Tensor) -> Tensor:
    ...
    quotient = shifted / 10.0
    # Straight-through floor: exact place value in forward, gradients through ``shifted``.
    floor_quotient = (torch.floor(quotient) - quotient).detach() + quotient
    remainder = shifted - 10.0 * floor_quotient
...
class MiniLeNetRegressor(nn.Module):
    ...
    def __init__(self, c1: int = 6, c2: int = 12, hidden: int = 16) -> None:
        super().__init__()
...
class WhyFC(nn.Module):
    """Conv decoder that reads re-synthesized digit images from WhyNeurons.

    Each WhyNeuron emits five 28x28 channels; a WhyLinearLayer stacks them into
    ``[batch, out_features * 5, 28, 28]`` for the next stage.
    """

    def __init__(self, in_channels: int, out_features: int = 10) -> None:
        super().__init__()
...
class WhyLeNet(nn.Module):
    """Compact MNIST classifier with one Why hidden stage and image/scalar heads."""

    def __init__(self, digit_bank: DigitBank, hidden_neurons: int = 8) -> None:
        super().__init__()
        ...
        image_channels = hidden_neurons * len(POSITION_WEIGHTS)
        self.why_fc = WhyFC(image_channels)
        self.scalar_head = nn.Sequential(
            nn.LayerNorm(hidden_neurons),
            nn.Linear(hidden_neurons, 10),
        )
        self.fusion = nn.Parameter(torch.tensor([1.0, 0.5, 0.25]))

    def _fuse_logits(self, encoder_logits: Tensor, image_logits: Tensor, scalar_logits: Tensor) -> Tensor:
        weights = F.softmax(self.fusion, dim=0)
        return weights[0] * encoder_logits + weights[1] * image_logits + weights[2] * scalar_logits

    def forward(
        self, x: Tensor, collect_stats: bool = False, return_encoder_logits: bool = False
    ) -> Tensor | Tuple[Tensor, EntropyStats] | Tuple[Tensor, Tensor] | Tuple[Tensor, Tensor, EntropyStats]:
        encoder_logits = self.encoder(x)
        if collect_stats:
            resynthesized, why_values, stats = self.why(encoder_logits, collect_stats=True)
            logits = self._fuse_logits(encoder_logits, self.why_fc(resynthesized), self.scalar_head(why_values))
            if return_encoder_logits:
                return logits, encoder_logits, stats
            return logits, stats
        resynthesized, why_values = self.why(encoder_logits)
        logits = self._fuse_logits(encoder_logits, self.why_fc(resynthesized), self.scalar_head(why_values))
        if return_encoder_logits:
...
def train_one_epoch(
    ...
    for images, labels in loader:
        ...
        logits, encoder_logits, batch_stats = model(images, collect_stats=True, return_encoder_logits=True)
        main_loss = F.cross_entropy(logits, labels)
        aux_loss = F.cross_entropy(encoder_logits, labels)
        ...
        total_loss += float(main_loss.detach().cpu()) * batch_size
...
def download_mnist(data_dir: Path) -> Tuple[datasets.MNIST, datasets.MNIST]:
    """Download MNIST train and test splits if they are not already cached."""
    ...
    print(f"MNIST ready in {data_dir.resolve()} ({len(train_full)} train, {len(test_full)} test)")
...
def build_loaders(data_dir: Path, batch_size: int, train_limit: int | None, test_limit: int | None) -> Tuple[datasets.MNIST, DataLoader, DataLoader]:
    train_full, test_full = download_mnist(data_dir)
    train_loader = DataLoader(
        maybe_subset(train_full, train_limit),
    ...
    test_loader = DataLoader(
        maybe_subset(test_full, test_limit),
        batch_size=batch_size,
        shuffle=False,
        num_workers=0,
        pin_memory=torch.cuda.is_available(),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--aux-weight", type=float, default=0.4, help="Weight for encoder auxiliary CE loss.")
    parser.add_argument("--hidden-neurons", type=int, default=8)
    ...
    parser.add_argument(
        "--download-only",
        action="store_true",
        help="Download MNIST into --data-dir and exit without training.",
    )
...
def main() -> None:
    ...
    train_full, train_loader, test_loader = build_loaders(
        args.data_dir, args.batch_size, args.train_limit or None, args.test_limit or None
    )
    ...
    for epoch in range(1, args.epochs + 1):
        train_metrics = train_one_epoch(model, train_loader, optimizer, device, args.aux_weight)
        eval_metrics = evaluate(model, test_loader, device)
        scheduler.step()
        best_test_acc = max(best_test_acc, eval_metrics["accuracy"])
        print(
            f"Epoch {epoch:03d} | "
    ...
    print(f"Saved checkpoint to {args.checkpoint} (best test acc {best_test_acc:.3%})")
